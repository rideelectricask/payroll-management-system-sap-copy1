...
import { fetchDriverData, apiCall } from "../services/api";
import FileUpload from "../components/FileUpload";
import { parseDriverExcelFile, validateExcelStructure } from "../utils/parseDriverExcel";
import LoadingSpinner from "../components/LoadingSpinner";
import ErrorAlert from "../components/ErrorAlert";
import SuccessAlert from "../components/SuccessAlert";
import { downloadDriverTemplate } from "../services/templateService";
...
<th className="px-4 py-2 text-left text-xs font-medium text-gray-700">Courier ID</th>
...
<td className="px-4 py-2 text-sm text-gray-700">{dup.data.username || '-'}</td>
<td className="px-4 py-2 text-sm text-gray-700">{dup.data.fullName || '-'}</td>
<td className="px-4 py-2 text-sm text-gray-700">{dup.data.courierId || '-'}</td>
...
<input type="text" value={localValue} onChange={handleChange} className="flex-1 px-2 py-1 border border-gray-300 rounded text-sm focus:outline-none focus:ring-1 focus:ring-blue-500" autoFocus={field === 'fee'} />
...
<input type="text" value={localValue} onChange={handleChange} className="w-full px-2 py-1 border border-gray-300 rounded text-sm focus:outline-none focus:ring-1 focus:ring-blue-500" autoFocus={field === 'business'} />
)}
</td>
);
}
...
const DriverRow = memo(({ item, isSelected, onSelect, editingItem, editFormData, onStartEdit, onEditChange, onSaveEdit, onCancelEdit, onDelete }) => {
...
const columns = [
{ key: 'business', width: 'px-6 py-4 whitespace-nowrap text-sm' },
{ key: 'unit', width: 'px-6 py-4 whitespace-nowrap text-sm' },
{ key: 'username', width: 'px-6 py-4 whitespace-nowrap text-sm' },
{ key: 'fullName', width: 'px-6 py-4 whitespace-nowrap text-sm' },
{ key: 'courierId', width: 'px-6 py-4 whitespace-nowrap text-sm' },
{ key: 'hubLocation', width: 'px-6 py-4 whitespace-nowrap text-sm' },
{ key: 'askor', width: 'px-6 py-4 whitespace-nowrap' },
{ key: 'fee', width: 'px-6 py-4 whitespace-nowrap text-sm' }
];
...
<span className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${item[col.key] === 'TRUE' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}>
{item[col.key]}
</span>
)}
</td>
...
if (col.key === 'fee') {
const displayValue = isEditing ? editFormData[col.key] : item[col.key];
const formattedValue = displayValue && displayValue !== '0' ? `Rp ${Number(displayValue).toLocaleString('id-ID')}` : '-';

return (
<EditableCell key={col.key} item={item} field={col.key} value={isEditing ? editFormData[col.key] : formattedValue} isEditing={isEditing} editingId={editingItem} onStartEdit={handleStartEdit} onChange={onEditChange} onSave={onSaveEdit} onCancel={onCancelEdit} className={`${col.width} text-gray-900 border-r border-gray-200 last:border-r-0`} isLastCell={colIndex === columns.length - 1} />
);
}
...
let filtered = state.drivers.filter(driver =>
...
return filtered;
}, [state.drivers, state.searchTerm, sortConfig]);

const paginatedDrivers = useMemo(() => {
const startIndex = (state.currentPage - 1) * state.itemsPerPage;
return filteredAndSortedDrivers.slice(startIndex, startIndex + state.itemsPerPage);
}, [filteredAndSortedDrivers, state.currentPage, state.itemsPerPage]);

const totalPages = Math.ceil(filteredAndSortedDrivers.length / state.itemsPerPage);
...
Baris: dup.row,
Username: dup.data.username || '',
'Full Name': dup.data.fullName || '',
'Courier ID': dup.data.courierId || '',
...
XLSX.writeFile(wb, `Duplicate_Report_${timestamp}.xlsx`);
...
try {
const result = downloadDriverTemplate();
...
'Courier ID': driver.courierId || '',
'Hub Location': driver.hubLocation || '',
Askor: driver.askor || '',
Fee: driver.fee || ''
}));
...
const wb = XLSX.utils.book_new();
XLSX.utils.book_append_sheet(wb, ws, 'Driver Data');
...
const timestamp = new Date().toISOString().slice(0, 10);
XLSX.writeFile(wb, `Driver_Data_Export_${timestamp}.xlsx`);

showSuccessNotification("Export Success", `Successfully exported ${dataToExport.length} driver records`);
} catch (error) {
showErrorNotification("Export Failed", error.message);
} finally {
updateState({ isExporting: false });
}
}, [filteredAndSortedDrivers, updateState]);

const fetchDrivers = useCallback(async () => {
try {
updateState({ isLoading: true, error: "" });
const driversData = await fetchDriverData();
updateState({ 
drivers: driversData || [], 
isLoading: false,
success: `Loaded ${driversData?.length || 0} driver records`
});
clearMessages();
} catch (err) {
console.error("Failed to fetch drivers:", err.message);
updateState({ 
error: `Failed to load driver data: ${err.message}`, 
isLoading: false 
});
clearMessages();
}
...
const parsedData = await parseDriverExcelFile(file);

updateState({ 
data: parsedData, 
isLoading: false,
success: `Successfully parsed ${parsedData.length} driver records from Excel file`
});
...
const response = await apiCall('/driver/upload', {
method: 'POST',
data: state.data,
timeout: 30000
});
...
}, [state.data, updateState, clearMessages, fetchDrivers]);
...
try {
await apiCall(`/driver/data/${editingItem}`, {
method: 'PUT',
data: editFormData
});

showSuccessNotification("Update Success", `Driver "${editFormData.fullName}" updated successfully`);
setEditingItem(null);
setEditFormData({});
await fetchDrivers();
} catch (error) {
...
showErrorNotification("Update Failed", `Failed to update driver: ${error.message}`);
}
}
}, [editingItem, editFormData, fetchDrivers]);
...
await apiCall(`/driver/data/${deleteTarget.id}`, {
method: 'DELETE'
});
showSuccessNotification("Delete Success", `Driver "${deleteTarget.name}" deleted successfully`);
} else {
await apiCall('/driver/data/bulk-delete', {
method: 'DELETE',
data: { ids: deleteTarget.ids }
});
showSuccessNotification("Delete Success", `Successfully deleted ${deleteTarget.count} drivers`);
setSelectedItems(new Set());
}
await fetchDrivers();
} catch (error) {
showErrorNotification("Delete Failed", `Failed to delete driver(s): ${error.message}`);
} finally {
setIsDeleting(false);
setShowDeleteConfirm(false);
setDeleteTarget(null);
}
}, [deleteTarget, fetchDrivers]);
...
setSelectedItems(new Set(filteredAndSortedDrivers.map(d => d._id)));
...
return paginatedDrivers.map(item => item._id);
}, [paginatedDrivers]);
...
<ConfirmationModal isOpen={showDeleteConfirm} title="Confirm Delete" message={deleteTarget?.type === 'single' ? `Are you sure you want to delete driver "${deleteTarget.name}"?` : `Are you sure you want to delete ${deleteTarget?.count} selected drivers?`} onConfirm={confirmDelete} onCancel={cancelDelete} confirmText="Yes, Delete" cancelText="Cancel" isLoading={isDeleting} />

<div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
<h1 className="text-2xl font-bold text-gray-800">Mitra Information System</h1>
<div className="flex items-center gap-3">
<div className="flex items-center gap-3 text-sm text-gray-600">
<span>Total Drivers: {state.drivers.length}</span>
{filteredAndSortedDrivers.length !== state.drivers.length && (
<span>| Filtered: {filteredAndSortedDrivers.length}</span>
)}
</div>
<button onClick={handleExportData} disabled={state.isExporting || state.drivers.length === 0} className="flex items-center gap-2 px-4 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 disabled:bg-gray-400 disabled:cursor-not-allowed transition-colors font-medium text-sm shadow-sm">
...
<h2 className="text-lg font-semibold mb-4">Upload Driver Data</h2>
...
{ key: 'hubLocation', label: 'Hub Location' },
{ key: 'askor', label: 'Askor' },
{ key: 'fee', label: 'Fee' }
].map(({ key, label }) => (
<th
key={key}
...
<td className="px-3 py-2 text-xs whitespace-nowrap border-r border-gray-200">{row.business || '-'}</td>
<td className="px-3 py-2 text-xs whitespace-nowrap border-r border-gray-200">{row.unit || '-'}</td>
<td className="px-3 py-2 text-xs whitespace-nowrap border-r border-gray-200">{row.username || '-'}</td>
<td className="px-3 py-2 text-xs whitespace-nowrap border-r border-gray-200">{row.fullName || '-'}</td>
<td className="px-3 py-2 text-xs whitespace-nowrap border-r border-gray-200">{row.courierId || '-'}</td>
<td className="px-3 py-2 text-xs whitespace-nowrap border-r border-gray-200">{row.hubLocation || '-'}</td>
...
{row.fee && row.fee !== '0' ? `Rp ${Number(row.fee).toLocaleString('id-ID')}` : '-'}
</td>
</tr>
))}
</tbody>
...
<BulkActionBar selectedItems={Array.from(selectedItems)} onBulkDelete={handleBulkDelete} onSelectAll={handleSelectAll} onDeselectAll={handleDeselectAll} totalItems={filteredAndSortedDrivers.length} />

<div className="bg-white rounded-lg shadow-sm !border-gray-300 border p-4">
<div className="flex flex-col sm:flex-row gap-4 items-center justify-between">
<div className="flex-1 max-w-md">
<input type="text" placeholder="Search drivers..." value={state.searchTerm} onChange={(e) => handleSearch(e.target.value)} className="w-full px-4 py-2 !border-gray-300 border rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent" />
</div>
...
<DriverRow key={driver._id} item={driver} isSelected={selectedItems.has(driver._id)} onSelect={handleSelectItem} editingItem={editingItem} editFormData={editFormData} onStartEdit={handleEdit} onEditChange={handleEditChange} onSaveEdit={handleSaveEdit} onCancelEdit={handleCancelEdit} onDelete={handleDelete} />
))}
{paginatedDrivers.length === 0 && (
<tr>
<td colSpan={10} className="px-6 py-8 text-center text-gray-500">
{state.searchTerm ? 'No drivers found matching your search.' : 'No driver data available.'}
</td>
</tr>
)}
</tbody>
...
{Math.min(state.currentPage * state.itemsPerPage, filteredAndSortedDrivers.length)} of{' '}
{filteredAndSortedDrivers.length} results
</div>
<div className="flex items-center gap-2">
