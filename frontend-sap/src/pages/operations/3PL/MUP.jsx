...
import { Package, TrendingUp, CheckCircle, Truck, Clock, MapPin, RefreshCw, Loader2, Building2, Factory, Upload, Download, Trash2, CheckSquare, Square, X } from "lucide-react";
...
const UploadNotification = ({ result, onClose }) => {
  if (!result) return null;
  return (
    <div className="rounded-xl border border-gray-200 bg-white shadow-sm overflow-hidden">
      <div className="flex items-center justify-between px-5 py-3 bg-gray-50 border-b border-gray-200">
        <span className="font-semibold text-gray-700 text-sm">Upload Result</span>
      ...
      <div className="p-4 space-y-2">
        {result.inserted > 0 && (
          <div className="flex items-start gap-3 p-3 bg-green-50 border border-green-200 rounded-lg">
            ...
            <div>
              ...
              <p className="text-xs text-green-700">{result.inserted} data berhasil diunggah.</p>
            </div>
          </div>
        )}
        {result.duplicateCount > 0 && (
          <div className="flex items-start gap-3 p-3 bg-red-50 border border-red-200 rounded-lg">
            ...
            <div>
              <p className="text-sm font-semibold text-red-800">Failed (Duplicate)</p>
              <p className="text-xs text-red-700">{result.duplicateCount} data dilewati karena merchant_order_id sudah ada di database.</p>
              {result.duplicateIds && result.duplicateIds.length > 0 && (
                <div className="mt-1 max-h-24 overflow-y-auto">
                  {result.duplicateIds.map(id => (
                    <span key={id} className="inline-block mr-1 mb-1 px-1.5 py-0.5 bg-red-100 text-red-700 rounded text-xs font-mono">{id}</span>
                  ))}
                </div>
              )}
            </div>
        ...
        {result.inserted === 0 && result.duplicateCount === 0 && (
          <p className="text-sm text-gray-500 text-center py-2">Tidak ada data yang diproses.</p>
        )}
      </div>
    </div>
  );
...
export default function MUP() {
  ...
  const statistics = useMemo(() => {
    ...
    const warehouses = new Set(orders.map(o => {
      ...
      if (s.includes('Jakarta') || s.includes('JK')) return 'Jakarta';
      if (s.includes('Tangerang') || s.includes('TNG')) return 'Tangerang';
      if (s.includes('Bekasi') || s.includes('BKS')) return 'Bekasi';
      if (s.includes('Depok') || s.includes('DPK')) return 'Depok';
      return 'Other';
    })).size;
    const routes = new Set(orders.map(o =>
      `${o.sender_name?.split('-')[0]?.trim() || 'Unknown'}-${o.destination_city || 'Unknown'}`
    )).size;
    return { total, delivered, inTransit, processing, destinations, totalWeight, warehouses: Math.max(warehouses, 1), activeRoutes: routes };
  }, [orders]);
  ...
  const warehouseData = useMemo(() => {
    ...
    orders.forEach(order => {
      ...
      if (s.includes('Jakarta Utara') || s.includes('JKT UTARA')) warehouse = 'Warehouse Jakarta Utara';
      ...
      else if (s.includes('Depok') || s.includes('DPK')) warehouse = 'Warehouse Depok';
      else if (s.includes('Jakarta')) warehouse = 'Warehouse Jakarta Utara';
  ...
  const performanceMetrics = useMemo(() => {
    if (orders.length === 0) return { avgDeliveryTime: 2.1, onTimeRate: 87, customerSatisfaction: 91 };
    ...
    const avgDeliveryTime = deliveredOrders.length > 0
      ...
      : 2.1;
    const onTimeRate = orders.length > 0 ? Math.round((deliveredOrders.length / orders.length) * 100) : 87;
    ...
    const customerSatisfaction = codOrders > 0 ? Math.round((successfulCod / codOrders) * 100) : 91;
  ...
  const loadOrders = useCallback(async () => {
    try {
      const response = await fetch(`${API_URL}/api/merchant-orders/mup/all`);
  ...
  const handleFileUpload = async (e) => {
    ...
    try {
      ...
      const response = await fetch(`${API_URL}/api/merchant-orders/mup/upload`, {
        method: 'POST',
  ...
  const handleBulkDelete = async () => {
    ...
    try {
      await Promise.all(Array.from(selectedOrders).map(id =>
        fetch(`${API_URL}/api/merchant-orders/mup/${id}`, { method: 'DELETE' })
      ));
  ...
  return (
    <div className="space-y-6">
      <div className="bg-gradient-to-r from-purple-500 to-pink-500 rounded-xl p-6 text-white">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-2xl font-bold">MUP Distribution</h2>
              <p className="text-sm opacity-90">Multi-Unit Project Management</p>
            </div>
          </div>
          <div className="flex gap-2">
            ...
            <input type="file" accept=".xlsx,.xls" onChange={handleFileUpload} className="hidden" id="file-upload-mup" disabled={isUploading} />
            <label htmlFor="file-upload-mup" className={`flex items-center gap-2 px-4 py-2 bg-white/20 hover:bg-white/30 rounded-lg transition-colors text-sm cursor-pointer ${isUploading ? 'opacity-50 cursor-not-allowed' : ''}`}>
      ...
      {uploadResult && (
        <UploadNotification result={uploadResult} onClose={() => setUploadResult(null)} />
      )}
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <div className="bg-white rounded-lg p-4 border border-gray-200 shadow-sm">
          <div className="flex items-center justify-between">
            <div><p className="text-gray-600 text-sm font-medium">Warehouses</p><p className="text-2xl font-bold text-purple-600">{statistics.warehouses}</p></div>
            <Factory className="w-8 h-8 text-purple-600 opacity-80" />
          </div>
        </div>
        <div className="bg-white rounded-lg p-4 border border-gray-200 shadow-sm">
          <div className="flex items-center justify-between">
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
          ...
          <div className="space-y-4">
            {warehouseData.length > 0 ? warehouseData.map((warehouse) => (
              <div key={warehouse.name} className="border-l-4 border-purple-500 bg-purple-50 p-4 rounded-r-lg">
                <div className="flex justify-between items-center mb-2">
                  <h4 className="font-semibold text-gray-800">{warehouse.name}</h4>
                  <span className="text-sm font-semibold text-purple-600">{warehouse.rate}%</span>
                </div>
                ...
                <div className="mt-2 w-full bg-gray-200 rounded-full h-2">
                  <div className="bg-purple-500 h-2 rounded-full" style={{ width: `${warehouse.rate}%` }}></div>
                </div>
              </div>
            )) : (
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between p-4 bg-blue-50 rounded-lg">
              <div><p className="text-sm text-gray-600">Active Routes</p><p className="text-2xl font-bold text-blue-600">{statistics.activeRoutes}</p></div>
              <Truck className="w-10 h-10 text-blue-600 opacity-80" />
            </div>
            <div className="flex items-center justify-between p-4 bg-green-50 rounded-lg">
              <div><p className="text-sm text-gray-600">Delivery Locations</p><p className="text-2xl font-bold text-green-600">{statistics.destinations}</p></div>
              <MapPin className="w-10 h-10 text-green-600 opacity-80" />
            </div>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
          <div className="bg-gray-50 px-6 py-4 border-b border-gray-200 flex items-center justify-between">
            <h3 className="text-lg font-bold text-gray-800">
              ...
              {selectedOrders.size > 0 && <span className="ml-2 text-sm font-normal text-purple-600">{selectedOrders.size} selected</span>}
            </h3>
            ...
            <table className="min-w-full divide-y divide-gray-200 text-sm">
              <thead className="bg-gray-50 sticky top-0 z-10">
                <tr>
                  <th className="px-3 py-3 text-left">
                    <button onClick={handleSelectAll} className="flex items-center justify-center w-5 h-5 text-purple-600 hover:text-purple-700">
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {orders.map((order, index) => (
                  <tr key={order._id || index} className={`hover:bg-gray-50 ${selectedOrders.has(order._id) ? 'bg-purple-50' : ''}`}>
                    <td className="px-3 py-3">
                      <button onClick={() => handleSelectOrder(order._id)} className="flex items-center justify-center w-5 h-5 text-purple-600 hover:text-purple-700">
                    ...
                    <td className="px-3 py-3 whitespace-nowrap">
                      <span className={`px-2 py-1 rounded-full text-xs font-medium ${order.payment_type === 'cod' ? 'bg-orange-100 text-orange-800' : 'bg-purple-100 text-purple-800'}`}>
