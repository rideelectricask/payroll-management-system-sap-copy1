...
import { Package, TrendingUp, CheckCircle, Truck, Clock, MapPin, RefreshCw, Loader2, Factory, Upload, Download, Trash2, CheckSquare, Square, X } from "lucide-react";
...
export default function JNE() {
  ...
  const statistics = useMemo(() => {
    ...
    const warehouses = new Set(orders.map(o => {
      ...
      if (s.includes('Bandung') || s.includes('BDG')) return 'Bandung';
      if (s.includes('Surabaya') || s.includes('SBY')) return 'Surabaya';
      if (s.includes('Semarang') || s.includes('SMG')) return 'Semarang';
      return 'Other';
    })).size;
  ...
  const warehouseData = useMemo(() => {
    ...
    orders.forEach(order => {
      ...
      else if (s.includes('Bandung') || s.includes('BDG')) warehouse = 'Warehouse Bandung';
      else if (s.includes('Surabaya') || s.includes('SBY')) warehouse = 'Warehouse Surabaya';
      else if (s.includes('Semarang') || s.includes('SMG')) warehouse = 'Warehouse Semarang';
  ...
  const performanceMetrics = useMemo(() => {
    if (orders.length === 0) return { avgDeliveryTime: 1.8, onTimeRate: 92, customerSatisfaction: 94 };
    ...
    const avgDeliveryTime = deliveredOrders.length > 0
      ...
      : 1.8;
    const onTimeRate = orders.length > 0 ? Math.round((deliveredOrders.length / orders.length) * 100) : 92;
    ...
    const customerSatisfaction = codOrders > 0 ? Math.round((successfulCod / codOrders) * 100) : 94;
  ...
  const loadOrders = useCallback(async () => {
    try {
      const response = await fetch(`${API_URL}/api/merchant-orders/jne/all`);
  ...
  const handleFileUpload = async (e) => {
    ...
    try {
      ...
      const response = await fetch(`${API_URL}/api/merchant-orders/jne/upload`, {
        method: 'POST',
  ...
  const handleBulkDelete = async () => {
    ...
    try {
      await Promise.all(Array.from(selectedOrders).map(id =>
        fetch(`${API_URL}/api/merchant-orders/jne/${id}`, { method: 'DELETE' })
      ));
  ...
  return (
    <div className="space-y-6">
      <div className="bg-gradient-to-r from-blue-500 to-indigo-500 rounded-xl p-6 text-white">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-2xl font-bold">JNE Operations</h2>
              <p className="text-sm opacity-90">JNE Logistics Management</p>
            </div>
          </div>
          <div className="flex gap-2">
            ...
            <input type="file" accept=".xlsx,.xls" onChange={handleFileUpload} className="hidden" id="file-upload-jne" disabled={isUploading} />
            <label htmlFor="file-upload-jne" className={`flex items-center gap-2 px-4 py-2 bg-white/20 hover:bg-white/30 rounded-lg transition-colors text-sm cursor-pointer ${isUploading ? 'opacity-50 cursor-not-allowed' : ''}`}>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <div className="bg-white rounded-lg p-4 border border-gray-200 shadow-sm">
          <div className="flex items-center justify-between">
            <div><p className="text-gray-600 text-sm font-medium">Warehouses</p><p className="text-2xl font-bold text-blue-600">{statistics.warehouses}</p></div>
            <Factory className="w-8 h-8 text-blue-600 opacity-80" />
          </div>
        </div>
        <div className="bg-white rounded-lg p-4 border border-gray-200 shadow-sm">
          <div className="flex items-center justify-between">
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
          ...
          <div className="space-y-4">
            {warehouseData.length > 0 ? warehouseData.map((warehouse) => (
              <div key={warehouse.name} className="border-l-4 border-blue-500 bg-blue-50 p-4 rounded-r-lg">
                <div className="flex justify-between items-center mb-2">
                  <h4 className="font-semibold text-gray-800">{warehouse.name}</h4>
                  <span className="text-sm font-semibold text-blue-600">{warehouse.rate}%</span>
                </div>
                ...
                <div className="mt-2 w-full bg-gray-200 rounded-full h-2">
                  <div className="bg-blue-500 h-2 rounded-full" style={{ width: `${warehouse.rate}%` }}></div>
                </div>
              </div>
            )) : (
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
          <div className="bg-gray-50 px-6 py-4 border-b border-gray-200 flex items-center justify-between">
            <h3 className="text-lg font-bold text-gray-800">
              ...
              {selectedOrders.size > 0 && <span className="ml-2 text-sm font-normal text-blue-600">{selectedOrders.size} selected</span>}
            </h3>
            ...
            <table className="min-w-full divide-y divide-gray-200 text-sm">
              <thead className="bg-gray-50 sticky top-0 z-10">
                <tr>
                  <th className="px-3 py-3 text-left">
                    <button onClick={handleSelectAll} className="flex items-center justify-center w-5 h-5 text-blue-600 hover:text-blue-700">
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {orders.map((order, index) => (
                  <tr key={order._id || index} className={`hover:bg-gray-50 ${selectedOrders.has(order._id) ? 'bg-blue-50' : ''}`}>
                    <td className="px-3 py-3">
                      <button onClick={() => handleSelectOrder(order._id)} className="flex items-center justify-center w-5 h-5 text-blue-600 hover:text-blue-700">
                    ...
                    <td className="px-3 py-3 whitespace-nowrap">
                      <span className={`px-2 py-1 rounded-full text-xs font-medium ${order.payment_type === 'cod' ? 'bg-orange-100 text-orange-800' : 'bg-blue-100 text-blue-800'}`}>
