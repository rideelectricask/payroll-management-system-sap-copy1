...
exports.uploadMerchantOrders = async (req, res) => {
  try {
    ...
    const incomingIds = uniqueData.map(item => item.merchant_order_id).filter(Boolean);
    const existingDocs = await Model.find({ merchant_order_id: { $in: incomingIds } }, { merchant_order_id: 1 });
    const existingIds = new Set(existingDocs.map(doc => doc.merchant_order_id));

    const newData = uniqueData.filter(item => !existingIds.has(item.merchant_order_id));
    const dbDuplicateIds = uniqueData.filter(item => existingIds.has(item.merchant_order_id)).map(item => item.merchant_order_id);

    const allDuplicateIds = [...dbDuplicateIds, ...internalDuplicateIds];

    let insertedCount = 0;
    if (newData.length > 0) {
      const bulkOps = newData.map(item => ({ insertOne: { document: item } }));
