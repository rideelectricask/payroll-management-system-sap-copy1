...
exports.uploadMerchantOrders = async (req, res) => {
  try {
    ...
    const incomingIds = data.map(item => item.merchant_order_id).filter(Boolean);
    ...
    const newData = data.filter(item => !existingIds.has(item.merchant_order_id));
    const duplicateData = data.filter(item => existingIds.has(item.merchant_order_id));

    let insertedCount = 0;

    if (newData.length > 0) {
      const bulkOps = newData.map(item => ({
    ...
    res.json({
      ...
      duplicateIds: duplicateData.map(item => item.merchant_order_id),
